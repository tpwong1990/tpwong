...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            long temp = round((image[i][j].rgbtBlue + image[i][j].rgbtGreen + image[i][j].rgbtRed) / 3);
            image[i][j].rgbtBlue = (BYTE) temp;
            image[i][j].rgbtGreen = (BYTE) temp;
            image[i][j].rgbtRed = (BYTE) temp;
        }
    }
    return;
}
